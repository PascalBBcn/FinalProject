...
public static class Pathfinding
{
    ...
    {
        if (!dungeonFloor.Contains(enemyPos) || !dungeonFloor.Contains(playerPos))
        {
        ...
        {
            return new List<Vector2Int> { enemyPos };
        }
        // Unvisited tiles
        ...
        SortedSet<(double, Vector2Int)> openList = new SortedSet<(double, Vector2Int)>(Comparer<(double, Vector2Int)>.Create((a, b) => a.Item1.CompareTo(b.Item1)));
        ...
        while (openList.Count > 0)
        {
            ...
            foreach (var dir in directions)
            {
                ...
                // Check if neighbourPos is playerPos
                if (neighbourPos == playerPos)
                {
    ...
    {
        // Euclidean distance (best for any directional movement)
        return Vector2Int.Distance(pos, playerPos);
    }
