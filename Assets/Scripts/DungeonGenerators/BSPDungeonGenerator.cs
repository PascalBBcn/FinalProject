...
public class BSPDungeonGenerator : MonoBehaviour
{
    ...
    private void CreateRooms()
    {
        ...
        int numOrganicRooms = 0;
        switch (GameSession.instance.currentFloor)
        {
            case 1:
                numOrganicRooms = 0;
                break;
            ...
            case 4:
                ...
                break;
            case 5:
                numOrganicRooms = Random.Range(2, 4);
                break;
            default:
                numOrganicRooms = 3;
                break;
        }
        // Elligible rooms to be considered as organic
        List<int> elligibleOrganicRoomIndices = new List<int>();
        for (int i = 0; i < roomData.Count; i++)
        {
            if (roomData[i].roomType == RoomType.Normal || roomData[i].roomType == RoomType.Boss) elligibleOrganicRoomIndices.Add(i);
        }
        // Store the chosen organic room indices
        List<int> organicRoomIndices = new List<int>();
        for (int i = 0; i < numOrganicRooms; i++)
        {
            int randomIndex = Random.Range(0, elligibleOrganicRoomIndices.Count);
            organicRoomIndices.Add(elligibleOrganicRoomIndices[randomIndex]);
            elligibleOrganicRoomIndices.RemoveAt(randomIndex); 
        }
        // Set these chosen rooms to organic
        foreach (var index in organicRoomIndices)
        {
            roomData[index].isOrganic = true;
        }

        // Rectangular or Organic rooms
        foreach (var room in roomData)
        {
            if (room.isOrganic)
            {
                var agentBasedRoom = PCGAlgorithms.AgentBasedDig(numberOfDigs, Vector2Int.FloorToInt(room.bounds.center));
                dungeonFloor.UnionWith(agentBasedRoom);
            }
            else
            {
                var rectRoom = CreateRectangularRoom(room.bounds);
                dungeonFloor.UnionWith(rectRoom);
            }
        }
    ...
    //     return dungeonFloor;
    // }
    private HashSet<Vector2Int> CreateRectangularRoom(RectInt room)
    {
        ...
        for (int col = padding; col < room.width - padding; col++)
        {
            for (int row = padding; row < room.height - padding; row++)
            {
                Vector2Int pos = new Vector2Int(room.x + col, room.y + row);
                floor.Add(pos);
            }
        }
        return floor;
